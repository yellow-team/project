...
public class StudentUnitRecordManager
{
    ...
    public void saveRecord(IStudentUnitRecord irec)
    {
        for (Element el : (List<Element>) XMLManager.getXML().getDocument().getRootElement().getChild("studentUnitRecordTable").getChildren("record"))
        {
            ...
            {
                el.setAttribute(Constants.ASG_1, new Float(irec.getasg1Mark()).toString());
                el.setAttribute(Constants.ASG_2, new Float(irec.getasg2Mark()).toString());
                el.setAttribute(Constants.EXAM, new Float(irec.getexamMark()).toString());
